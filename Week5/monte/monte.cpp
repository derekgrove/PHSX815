...
double monte_integrate(double(*f)(double), double a, double b, int n) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (strncmp(argv[i], "-b", 2) == 0) {
            ...
            if (strncmp(argv[i], "-m_intv", 3) == 0) {
                i++;
                int mn = std::stoi(argv[i]);
                if (mn > 0) {
                    monte_intervals = mn;
                }
            }
        ...
        double monte_result = monte_integrate(&my_function, a, b, monte_intervals);
        if (type == 0) {
            std::cout << "The numerical integration of y = x^5 + 8 from " << a << " to " << b << " with " << intervals
                      << " intervals is: " << integral << std::endl;
        }
        else {
            std::cout << "The quadrature integration of y = x^5 + 8 from " << a << " to " << b << " with " << quadrature
                      << " quadrature is: " << integral << std::endl;
        }
            std::cout << "for comparison, the analytic result for the integral is x^6/6 + 8x + C, and with limits " << a
                      << " to " << b << " we get result: " << analytic_result << " + C" << std::endl;

        std::cout
                << "for even more comparison, here is the monte carlo integration result for the same integration bounds and function: "
                << monte_intervals << " number of random points: " << monte_result << std::endl;
        return 0;
    }
